...
Page({
  ...
  onShow:function(){
    ...
    app.ajax('userCart', {}, function (d) {
      if (d.code == 1) {
        ...
        // wx.setStorageSync('cart_num', that.data.pdt_num)
      }else{
  ...
  itemSetNum:function(index){
    ...
    pm.num = item.num;
    pm.addtype = 1;
    app.ajax('userAddCart', pm, function (d) {
      if (d.code == 1) {
  ...
  del_item: function (e) {
    ...
    var pm = {};
    pm.pid = item.id;
    app.ajax('userDelCart', pm, function (d) {
      if (d.code == 1) {
  ...
  submit_cart:function(e){
    ...
    app.ajax('userCartSub',{pids:pida.join(',')},function(d){
      if (d.code==1) {
        wx.navigateTo({
